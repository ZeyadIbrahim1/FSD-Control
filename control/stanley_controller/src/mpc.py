...
def waypoints_callback(waypoints_msg):
    ...
    for point in waypoints_msg.markers[0].points:
      waypoints_of_x.append(point.x)
      waypoints_of_y.append(point.y)
...
def main():
    ...
    cyaw = []
    ck = []
    z = len(waypoints_of_x)

    for i in range(z):
        cx.append(waypoints_of_x[i]) 
        cy.append(waypoints_of_y[i]) #for future reference
    ...
    control_msg.drive.steering_angle = steer ##variable steering to be changed to variable 
    control_msg.drive.speed = sp  ##variable speed to be changed done
